...
var isCleanForce = function() {
  return argv.cleanForce;
};
...
gulp.task('scripts.modules', function() {
  ...
  dirs.forEach(function(dir) {
    ...
    if (exportsConfig) {
      ...
      exportsJs += exportsConfigJs.join('');
    }
    ...
    if (exportsLocale) {
      ...
      exportsJs += exportsLocaleJs.join('');
    }
    ...
    sortConfig = {
      comparator: function(a, b) {
        ...
        if (getNormalizedName(a) == getNormalizedName(b)) {
          if (isSpecial(a) && isSpecial(b)) {
            return getSpecialType(a) == 'debug'
              ? -1
              : 0;
          }
          return isSpecial(a)
        ...
        function isSpecial(p) {
          return ['.local.js', '.debug.js'].indexOf(p.slice(-9)) >= 0;
        }
        function getNormalizedName(p) {
          return isSpecial(p)
            ? p.slice(0, -8) + 'js'
            : p;
        }
    ...
    tasks.push(
      streamqueue({ objectMode: true },
        gulp.src(['module.js'], { cwd: base })
          .pipe(footer(exportsJs)),
        gulp.src(
          [
            'config/**/*.js',
            '!**/!*.js',
